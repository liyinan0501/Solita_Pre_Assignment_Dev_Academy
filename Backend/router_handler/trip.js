...
exports.getTrips = (req, res) => {
  ...
  if (!departureStationId) {
    const getAllTrips = `select *, count(*) over() as totalCount from journeys limit ${startRow}, ${pageSize}`
    ...
    const searchTrips = `select *, count(*) over() as totalCount from journeys where departure_station_id = ${departureStationId} and departure >= '${departureDate}' and \`return\` <= '${returnDate}' order by departure asc limit ${startRow}, ${pageSize}`
